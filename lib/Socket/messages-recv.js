...
export const makeMessagesRecvSocket = (config) => {
    ...
    const accepteCall = async (callId, callFrom, isVideo) => {
		const meId = authState.creds.me?.id
		const acceptContent = [{ tag: 'audio', attrs: { rate: '16000', enc: 'opus' }, content: undefined }]
		if (isVideo) {
    ...
    const endCall = async(callId, from) => {
		const stanza = ({
			tag: 'call',
			attrs: {
